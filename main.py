...
TMDB_IMAGE_500 = "https://image.tmdb.org/t/p/w500"

if not TMDB_API_KEY:
    raise RuntimeError("Please provide a TMDB API key in the .env file")

app = FastAPI(
    title="Movie Recommendation System",
    description="A movie recommendation system using TMDB API and cosine similarity",
    version="1.0.0",
)
...
#On start of API all the pickle files will be load
...
#Movie Details when we click on any movie
@app.get("/movie/id/{tmdb_id}", response_model=TMDBMovieDetails)
async def movie_details_route(tmdb_id: int):
    return await tmdb_movie_details(tmdb_id)


#Display list based on genre
...
#Fetch Movie titles and similarity scores
...
#When we click on any movie then it recommend movies list based on tfidf and genre
